...
```json
{
  "mode": "research_discovery | deep_reproduction",
  "papers": [
    {
  ...
  "prompt": {
    "research_goal": "请调研长上下文 Transformer 的主要效率路线，并判断哪些方向还有创新空间",
    "user_idea": "是否可以把状态空间模型和线性注意力结合做统一框架？",
    "is_new_domain": true
  },
  "config": {
    "mode_1": {
      ...
      "innovation_hypotheses": ["hybrid ssm-attention", "retrieval-augmented efficiency"],
      "deliver_frontier_report": true
    },
    "mode_2": {
...
- `mode`：必填。`research_discovery` 对应 Mode 1，`deep_reproduction` 对应 Mode 2。
- `papers`：Mode 1 与 Mode 2 都可提供。至少一篇时，`pdf_url` 和 `pdf_path` 二选一，系统优先使用本地路径。`role` 在 Mode 1 中为 `seed`，Mode 2 中为 `target`。
- `prompt`：Mode 1 可选但推荐。用于直接提供研究目标、文字叙述、用户自带 idea 和是否为陌生领域；没有种子论文时可仅靠 `prompt` 启动调研发现流程。
- `config.mode_1`：Mode 1 专属配置。`domain_context` 帮助 agent 判断论文相关性；`max_depth` 控制递归探索深度；`focus_directions` 和 `ignore_directions` 引导探索方向；`innovation_hypotheses` 记录用户或系统关注的潜在创新点；`deliver_frontier_report` 控制是否默认产出研究史与前沿趋势报告。
...
> [!note]
> 当前仓库实现层尚未把 Mode 1 的实际 API/枚举从旧命名迁移到 `research_discovery`。本 RFC 描述的是目标接口语义，代码实现需后续补迁移。
...
```json
{
  ...
  "data": {
    ...
    "report_path": "reports/exploration/discovery-report.md",
...
- **planning**：根据 mode 和 config 生成执行计划。Mode 1 生成调研发现计划（问题澄清、扩展方向、调研产物、预算分配）；Mode 2 生成复现计划（实现步骤、实验矩阵、里程碑）。
...
- `clarify_research_goal`：根据种子材料或文字叙述澄清问题边界
- `ingest_paper`：解析一篇论文，生成 PaperCard
- `prioritize_references`：对 PaperCard 的参考文献排优先级
- `explore_paper`：探索一篇新论文（解析 + 结构化阅读）
- `update_knowledge_graph`：补充概念、方法、证据与论文关系
- `summarize_method_cluster`：总结一个方法簇的实验范式与代表路线
- `evaluate_user_idea`：对用户自带 idea 生成可行性分析
- `propose_idea_directions`：产出值得继续构思的潜在方向
- `check_termination`：评估终止条件（递减收益检测）
- `generate_discovery_report`：生成最终调研发现报告
...
```json
{
  ...
  "checkpoint": {
    ...
    "completed_steps": [
      ...
      {"step": "summarize_method_cluster", "cluster": "linear-attention", "result": "success", "at": "..."},
      {"step": "evaluate_user_idea", "idea": "hybrid ssm-attention", "decision": "promising", "at": "..."}
    ],
    "pending_queue": [
      {"step": "propose_idea_directions"},
...
- **Web UI**：V1 核心 RFC 不把前端视为 orchestrator 的一部分；主线仍是 API-first，CLI 客户端作为参考实现。独立工作台设计见 [[framework/webui-v1-rfc]]。
...
- **Web UI**：基于 SSE 事件流的实时仪表盘，展示任务进度、工件图谱、预算消耗。WebUI-v1 的独立规格见 [[framework/webui-v1-rfc]]。
