...
from ainrf.terminal.models import TerminalAttachmentTarget, TerminalMuxKind, TerminalSessionStatus, UserEnvironmentBinding, utc_now
...
def make_manager(tmp_path: Path, *, user_id: str = "daemon-user") -> tuple[SessionManager, InMemoryEnvironmentService]:
...
def test_binding_upsert_is_idempotent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(adapter, "build_attach_command", lambda *args, **kwargs: ("tmux", "attach-session"))
...
def test_tmux_adapter_builds_local_commands(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        adapter,
        "_run_local_command",
        lambda command: captured.append(command) or SimpleNamespace(returncode=0, stdout="", stderr=""),
    )
...
def test_tmux_adapter_builds_remote_commands(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    adapter = TmuxAdapter(tmp_path)
    environment = InMemoryEnvironmentService().create_environment(
        alias="remote-lab",
    ...
    monkeypatch.setattr(
        adapter,
        "_run_remote_command",
        lambda env, user, remote_command: captured.append((user, remote_command))
...
def test_terminal_attachment_broker_validates_token_and_expiry(tmp_path: Path) -> None:
    ...
    broker._attachments[attachment.attachment_id].expires_at = (
        attachment.expires_at - timedelta(minutes=10)
    )
...
def test_reset_kills_and_recreates_personal_session(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    adapter = TmuxAdapter(tmp_path)
    environment = InMemoryEnvironmentService().create_environment(
        alias="gpu-lab",
        display_name="GPU Lab",
...
def test_reconcile_restores_running_state_from_sqlite(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager, environment_service = make_manager(tmp_path)
    environment = environment_service.create_environment(
        alias="gpu-lab",
        display_name="GPU Lab",
...
def manager_binding(environment_id: str, *, remote_login_user: str = "root") -> UserEnvironmentBinding:
    timestamp = utc_now()
