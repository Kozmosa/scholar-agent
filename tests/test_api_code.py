...
@pytest.mark.anyio
@pytest.mark.parametrize("path", ["/code/", "/v1/code/"])
async def test_code_proxy_returns_503_when_upstream_times_out(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    path: str,
) -> None:
    ...
    assert response.status_code == 503
    assert response.json() == {"detail": "code-server upstream timed out"}


@pytest.mark.anyio
@pytest.mark.parametrize("path", ["/code/", "/v1/code/"])
async def test_code_proxy_returns_502_when_upstream_transport_fails(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    path: str,
) -> None:
    ...
    assert response.json() == {"detail": "code-server upstream request failed"}
